...
class GaitEstimation(nn.Module):

    def __init__(self,cfgs,training):
        super(GaitEstimation, self).__init__()
        ...
        if training:
            self.gait_loss_aggregator = LossAggregator(cfgs['loss_cfg'])
    ...
    def save_ckpt(self, iteration):
        ...
        mkdir(osp.join(self.save_path, "gait_checkpoints/"))
        ...
        torch.save(checkpoint,
                   osp.join(self.save_path, 'gait_checkpoints/{}-{:0>5}.pt'.format(save_name, iteration)))
        
    def resume_ckpt(self, restore_hint):
        if isinstance(restore_hint, int):
            save_name = self.engine_cfg['save_name']
            save_name = osp.join(
                self.save_path, 'gait_checkpoints/{}-{:0>5}.pt'.format(save_name, restore_hint))
    ...
    def forward(self,features,labs,seqL):
