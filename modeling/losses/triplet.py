...
class TripletLoss(BaseLoss):
    ...
    def ComputeDistance(self, x, y):
        ...
        dist = x2 + y2 - 2 * inner
        epsilon = 1e-8 
        dist = torch.sqrt(F.relu(dist)+epsilon)
...
class DirectionTripletLoss(BaseLoss):
    def __init__(self, margin, loss_term_weight=1.0):
        super(DirectionTripletLoss, self).__init__(loss_term_weight)
        self.margin = margin
    ...
    def ComputeDistance(self, x, y):
        ...
        dist = x2 + y2 - 2 * inner
        epsilon = 1e-8 
        dist = torch.sqrt(F.relu(dist)+epsilon)  # [p, n_x, n_y]
        # print(dist)
        return dist
